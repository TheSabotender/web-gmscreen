...
export class Die extends DiceTerm {
    ...
    reroll(modifier, { recursive = false } = {}) {
        ...
        max = foundry.utils.isNumeric(max) ? parseInt(max) : null;
        target = foundry.utils.isNumeric(target) ? parseInt(target) : 1;
    ...
    explode(modifier, { recursive = true } = {}) {
        ...
        // Determine target values
        target = foundry.utils.isNumeric(target) ? parseInt(target) : this.faces;
        comparison = comparison || "=";
