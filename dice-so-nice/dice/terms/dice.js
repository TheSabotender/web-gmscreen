...
export class DiceTerm extends RollTerm {
  ...
  alter(multiply, add) {
    ...
    multiply = foundry.utils.isFinite(multiply) && (multiply >= 0) ? multiply : 1;
    add = foundry.utils.isInteger(add) ? add : 0;
  ...
  static fromMatch(match) {
    ...
    number = foundry.utils.isNumeric(number) ? parseInt(number) : 1;
    denomination = foundry.utils.isNumeric(denomination) ? parseInt(denomination) : 6;
