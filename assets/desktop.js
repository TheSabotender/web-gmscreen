...
function createBackgroundVideoWrapper(state) {
  ...
  wrapper.className = 'wallpaper-video-wrapper';
  const mode = state.settings.backgroundMode || 'envelop';
  wrapper.dataset.mode = mode;

  const video = document.createElement('video');
  video.className = 'wallpaper-bg-video';
  video.src = url;
  video.loop = true;
  video.autoplay = true;
...
export function renderWallpaper(context) {
    ctx = context;
    const state = context.getState();
    const { wallpaper } = context.elements;
    if (!wallpaper) return;
    
    wallpaper.innerHTML = '';  
    const videoWrapper = createBackgroundVideoWrapper(state);
    if (videoWrapper) {
        wallpaper.appendChild(videoWrapper);
    } else {
